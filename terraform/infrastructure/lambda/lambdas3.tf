...
resource "aws_s3_bucket_object" "lambda_listingHistory" {
  bucket = data.aws_s3_bucket.neocar_lambda.bucket
  key    = "v1.0.0/listingHistory.zip"
  source = "../../artifacts/lambda/listingHistory.zip"

  # The filemd5() function is available in Terraform 0.11.12 and later
  # For Terraform 0.11.11 and earlier, use the md5() function and the file() function:
  # etag = "${md5(file("path/to/file"))}"
  etag = filemd5("../../artifacts/lambda/listingHistory.zip")
}
